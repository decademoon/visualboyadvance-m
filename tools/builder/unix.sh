...
table_insert_before DISTS wxwidgets '
    ...
    gtk+3           https://download.gnome.org/sources/gtk+/3.22/gtk+-3.22.26.tar.xz                            lib/libgtk-3.a
...
table_line_append  DIST_PRE_BUILD       gtk+3 "sed -i.bak 's/^\\(SRC_SUBDIRS = gdk gtk libgail-util\\).*/\\1/; s/^\\(SUBDIRS = .*\\) docs\\( .*\\)/\\1\\2/' Makefile.am;"
