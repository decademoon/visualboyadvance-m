...
build_dist() {
    ...
    if [ -n "$build_override" ]; then
        ...
        if [ -z "$config_type" -o "$config_type" = meson ] && [ -f meson.build ]; then
            mkdir -p build
            cd build
            ...
            path_exists "$(install_artifact $current_dist)"
        elif [ -z "$config_type" -o "$config_type" = autoconf -o "$config_type" = autoreconf ] && [ -f configure -o -f Configure -o -f configure.ac -o -f configure.in -o -f Makefile.am ]; then
            ...
            path_exists "$(install_artifact $current_dist)"
        elif [ -z "$config_type" -o "$config_type" = cmake ] && [ -f CMakeLists.txt ]; then
            mkdir -p build
            cd build
            ...
            path_exists "$(install_artifact $current_dist)"
        elif [ -z "$config_type" -o "$config_type" = python ] && [ -f setup.py ]; then
            if [ -z "$install_override" ]; then
                pip=
            ...
            path_exists "$(install_artifact $current_dist)"
        elif [ -z "$config_type" -o "$config_type" = perl ] && [ -f Makefile.PL ]; then
            ...
            path_exists "$(install_artifact $current_dist)"
        elif [ -z "$config_type" -o "$config_type" = make ] && [ -f Makefile -o -f makefile ]; then
            makefile=makefile
            if [ -f Makefile ]; then
                makefile=Makefile
            fi
